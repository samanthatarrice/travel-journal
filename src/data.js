export default [
  {
    id: 1,
    image: "patagonia.jpeg",
    alt: "Sam and her husband in front of Perito Moreno Glacier",
    country: "Argentina",
    googleMapsUrl: "https://goo.gl/maps/NnT9CLpX8AvQCxrJ6",
    title: "Perito Moreno Glacier",
    date: "October 2018",
    description: "Visiting Patagonia in Argentina was an amazing trip. We were about a mile away from hiking all the way up Monte Fitz Roy, the mountains you see on the Patagonia company logo. But, we decided not because the conditions were icey and I was five months pregnant. It was still an incredible hike. We also see glaciers up close, and I've never been more freezing in my life. A few times we could see ice melting off, which was exciting, but seeing how far the glaciers had receeded was a real eye opener."
  },
  {
    id: 2,
    image: "mexico.jpeg",
    alt: "Sam standing in front of Teotihuacan.",
    country: "Mexico",
    googleMapsUrl: "https://goo.gl/maps/9MvP39phP9f7q1VV7",
    title: "Teotihuacan",
    date: "July 2018",
    description: "Mexico City is one of my favorite cities. There is so much great history, culture, and food, and so much to explore! I could definitely see my family possibly living there one day. This was taken on the ruins of Teotihuacan in a small town outside of CDX."
  },
  {
    id: 3,
    image: "peru.jpeg",
    alt: "Sam standing face to face with a llama in Machu Picchu.",
    country: "Peru",
    googleMapsUrl: "https://goo.gl/maps/Tzmdkh94ezEP2ckc7",
    title: "Machu Picchu",
    date: "January 2018",
    description: "Peru is such an icredible country with so much history, friendly people, and a diverse landscape. My husband and I explored the country, and of course visited the Sacred Valley and Machu Picchu. Here I am with one of the llamas that roam free around the ruins."
  },
  {
    id: 4,
    image: "salvador.jpeg",
    alt: "Sam sitting in a hammock in Boipeba.",
    country: "Brazil",
    googleMapsUrl: "https://goo.gl/maps/X2qvsYs29V3tVVYe9",
    title: "Boipeba",
    date: "October 2017",
    description: "My husband and I moved to Salvador, Brazil in August 2017 to teach at an international school. That became the jumping off place to our other adventures in South America. To get to Boipeba from Salvador, you first need to take a ferry to a small town across the bay. From there you find a taxi to drive you to another small town an hour away. Then, you have to wait for a speedboat to drive you an hour down the coast. The trip is more than worth it though. I've never experienced such beautiful remote beach getaway like this!"
  },
  {
    id: 5,
    image: "paris.jpeg",
    alt: "Sam standing in front of the Eiffel Tower.",
    country: "France",
    googleMapsUrl: "https://goo.gl/maps/Tzmdkh94ezEP2ckc7",
    title: "Eiffel Tower",
    date: "Summer of 2008",
    description: "This was a solo trip to southern Europe that started my wanderlust. After graduating college, I decided to celebrate by spending a little over a month exploring Italy, Switzerland, France, and Spain."
  }
]